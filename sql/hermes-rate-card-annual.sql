...
WHERE cruise_name = '헤르메스 크루즈'
ORDER BY CASE WHEN room_type = '주니어 스위트 발코니' THEN 1 WHEN room_type = '주니어 스위트 트리플' THEN 2 WHEN room_type = '주니어 스위트 패밀리 커넥팅' THEN 3 WHEN room_type = '시니어 스위트 발코니' THEN 4 WHEN room_type = '시니어 스위트 트리플' THEN 5 WHEN room_type = '시니어 스위트 패밀리 커넥팅' THEN 6 WHEN room_type = '로얄 스위트 위드 테라스' THEN 7 ELSE 8 END;

-- 휴일 추가요금 요약
SELECT '=== 헤르메스 크루즈 | 휴일 추가요금 ===' AS status;
...
SELECT 
  (SELECT COUNT(*) FROM cruise_rate_card WHERE cruise_name = '헤르메스 크루즈') as "객실가격행 (8행)",
  (SELECT COUNT(*) FROM cruise_holiday_surcharge WHERE cruise_name = '헤르메스 크루즈') as "휴일추가행 (3행)";

-- 발코니 객실 확인
SELECT '=== 헤르메스 크루즈 | 발코니 객실 ===' AS status;
