-- 고객 요청사항 시스템 - UUID 타입 안전 버전
-- 2025.08.21 - UUID/VARCHAR 타입 불일치 해결
...
BEGIN
  ...
  UPDATE customer_requests SET
    ...
    processed_by = CASE WHEN p_new_status = 'completed' THEN auth.uid()::uuid ELSE processed_by END,
...
-- 테이블 코멘트
COMMENT ON TABLE customer_requests IS '고객 요청사항 관리 테이블 - UUID 타입 안전 버전';
...
-- 완료 메시지
SELECT 'Customer requests system created with UUID type safety' as result;
