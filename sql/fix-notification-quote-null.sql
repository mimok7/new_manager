-- 알림 함수 수정: quote_rec 미할당 오류 해결 + phone_number 사용
-- 에러: record "quote_rec" is not assigned yet (55000)
-- 원인: re_quote_id가 NULL인 경우 quote_rec 변수가 할당되지 않은 상태에서 접근

BEGIN;

-- 1. create_business_notification 함수
...
-- 2. create_reservation_notification 함수 수정 (quote_rec 접근 오류 해결)
...
DECLARE
    ...
    message_text TEXT;
    service_name TEXT;
    quote_title TEXT := '연결된 견적 없음';  -- ✅ 기본값 설정
BEGIN
    -- 예약 정보 조회
    ...
    service_name := CASE reservation_rec.re_type
        ...
        ELSE COALESCE(reservation_rec.re_type, '기타')
    END;
...
확인 및 처리 부탁드립니다.',
        ...
        COALESCE(user_rec.phone_number, '연락처없음'),
        service_name,
        quote_title,  -- ✅ 안전하게 처리된 변수 사용
        ...
        CASE reservation_rec.re_status
            ...
            ELSE COALESCE(reservation_rec.re_status, '알수없음')
        END
    );
...
-- 3. create_payment_notification 함수 수정
...
-- 4. create_quote_approval_notification 함수 수정
...
검토 후 승인 처리 부탁드립니다.',
        ...
        CASE quote_rec.status 
            ...
            ELSE COALESCE(quote_rec.status, '알수없음')
        END
    );
...
SELECT '알림 함수 수정 완료: quote_rec 미할당 오류 해결' as result;
