-- ============================================================================
-- Indochina Premium Cruise - Rate Card (신용카드 / VND 송금)
-- 인도차이나 프리미엄 크루즈 - 객실요금 데이터
-- ============================================================================
-- 입력 기준 데이터:
--   시즌1: 2026/01/01 - 09/30 (성수기)
...
-- ============================================================================
-- Section 1: Season 1 (2026/01/01 - 09/30) - 신용카드
-- ============================================================================
...
-- ============================================================================
-- Section 2: Season 1 (2026/01/01 - 09/30) - VND 송금
-- ============================================================================
...
SELECT 
  (SELECT COUNT(*) FROM cruise_rate_card WHERE cruise_name = '인도차이나 프리미엄 크루즈') as "객실가격행 (16행 예상)",
  (SELECT COUNT(*) FROM cruise_holiday_surcharge WHERE cruise_name = '인도차이나 프리미엄 크루즈') as "휴일추가행 (3행 예상)";
...
SELECT 
  DATE_TRUNC('month', valid_from)::DATE as "시즌_시작",
  COUNT(*) as "객실행수 (8행 예상)"
FROM cruise_rate_card
WHERE cruise_name = '인도차이나 프리미엄 크루즈'
...
-- 유아 가격 검증 (모두 4,250,000이어야 함)
SELECT '=== 유아 가격 검증 (4,250,000 통일) ===' AS status;

SELECT 
  COUNT(*) as "총행수",
  COUNT(DISTINCT price_infant) as "유아가격 종류",
  MAX(price_infant) as "최대유아가",
  MIN(price_infant) as "최소유아가"
FROM cruise_rate_card
WHERE cruise_name = '인도차이나 프리미엄 크루즈';
