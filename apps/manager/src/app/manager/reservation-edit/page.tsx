...
function ReservationEditContent() {
    ...
    const handleAddReservationForGroup = (reservation: ReservationSummary) => {
        if (reservation.re_quote_id) {
            router.push(`/manager/reservation-edit/new?quote_id=${reservation.re_quote_id}`);
            return;
        }
        if (reservation.users?.id) {
            router.push(`/manager/reservation-edit/new?user_id=${reservation.users.id}`);
            return;
        }
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-lg shadow-sm p-6">
                    <div className="flex flex-wrap gap-4 items-center mb-4">
                        {/* 상태 필터 카드 */}
                        <div className="bg-blue-50 rounded-lg px-4 py-3 flex flex-wrap gap-2 items-center border border-blue-100">
                            <span className="text-sm font-medium text-gray-700">상태:</span>
                            {['all', 'pending', 'approved', 'confirmed', 'processing', 'cancelled', 'completed'].map((status) => (
                                <button
                                    key={status}
                        ...
                        {/* 새 예약 추가 */}
                        <button
                            onClick={() => router.push('/manager/reservation-edit/new')}
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    {[...reservation.services]
                                                        .sort((a, b) => getServiceSortOrder(a.re_type) - getServiceSortOrder(b.re_type))
                                                        .map((service, idx) => (
                                                            <span key={idx} className={`text-xs font-medium ${getStatusColor(service.re_status)}`}>
                                        ...
                                        <div className="p-4 border-t border-gray-100">
                                            <div className="flex flex-col gap-2">
                                                <button
                                                    ...
                                                    className="w-full inline-flex items-center justify-center gap-1 px-3 py-2 text-xs bg-green-100 text-green-700 rounded-md hover:bg-green-200 transition-colors"
                                                >
