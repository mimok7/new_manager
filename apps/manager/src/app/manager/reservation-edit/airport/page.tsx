...
function AirportReservationEditContent() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const hasAnyAirportData = (form: AirportWayFormData) => {
                return Boolean(
                    form.ra_airport_name ||
                    form.accommodation_info ||
                    form.ra_flight_number ||
                    form.ra_datetime ||
                    form.ra_request_note ||
                    form.ra_car_count > 0 ||
                    form.ra_passenger_count > 0 ||
                    form.unit_price > 0 ||
                    form.total_price > 0
                );
            };
            ...
            for (const way of wayKeys) {
                ...
                // 변경: 공항명은 ra_airport_location에 저장, 승차/하차 위치는 accommodation_info에 저장
                const payload = {
            ...
            // 1.5. 메인 예약 테이블 동기화 (총 금액 + 인원수 + 타임스탬프)
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                            공항 서비스 예약 수정
                            {fastTrackSummary.total > 0 && (
                                <span className="px-2 py-0.5 rounded-full bg-rose-100 text-rose-700 text-xs font-semibold">FAST TRACK</span>
                            )}
                        </h1>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-lg shadow-sm p-6">
                            ...
                            <div className="space-y-3">
                                ...
                                {fastTrackSummary.total > 0 && (
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">패스트랙 신청</label>
                                        <div className="text-rose-700 text-sm font-semibold">
                                            총 {fastTrackSummary.total}명 (픽업 {fastTrackSummary.pickup}명 / 샌딩 {fastTrackSummary.sending}명)
                                        </div>
                                        <div className="text-xs text-rose-700 mt-1">
                                            비용: {fastTrackUsdTotal.toLocaleString()}$ / {fastTrackKrwTotal.toLocaleString()}원
                                        </div>
                                    </div>
                                )}
                                <div className="pt-4 mt-4 border-t border-gray-100 space-y-4">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 mt-1">예약 상태 아래에서 추가요금을 조정하면 전체 예약 합계가 즉시 업데이트됩니다.</p>
                                    </div>
                                    <div>
