...
const RESERVATION_BATCH_SIZE = 80;
        ...
        const loadData = async () => {
            try {
                ...
                for (const batch of chunkArray(reservationIds, RESERVATION_BATCH_SIZE)) {
                    ...
                    reservationRows = reservationRows.concat(rows || []);
                }

                const reservationUserIdById = new Map<string, string>();
                const fallbackNameByReservationId = new Map<string, string>();
                const fallbackEmailByReservationId = new Map<string, string>();
                ...
                if (userIds.length > 0) {
                    for (const batch of chunkArray(userIds, RESERVATION_BATCH_SIZE)) {
                ...
                const mergedRows = baseRows.map((row) => {
                    ...
                    const userEmail = (userId && userEmailById.get(userId)) || fallbackEmailByReservationId.get(reservationId) || '';
                    const userName = (userId && userNameById.get(userId)) || fallbackNameByReservationId.get(reservationId) || '';
                    return {
                        ...row,
                        user_name: userName,
